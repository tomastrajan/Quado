...
public class ScoreDisplayHitEffect extends BasicEntity implements HitEffect {
    ...
    public ScoreDisplayHitEffect(int x, int y, int width, int height, Paint paint) {
        ...
        state = START_STATE;
        effectSizeMultiplier = (float) (((height * 0.5) / START_STATE) * ((Math.random() * 0.25) + 0.75));
        this.paint = paint;
        zIndex = -1;
    }
    ...
    public void render(MainGamePanel game, Canvas canvas, GameState gameState) {
        ...
        paint.setColorFilter(MyColors.getFilterScoreDisplayHitEffect());
        paint.setColor(MyColors.getBackgroundGradientLighter());

        //int alpha = (int) Math.ceil(state * (255 / (float) START_STATE));
        paint.setAlpha(255);
        ...
        canvas.clipRect(new Rect(left, top, right, bottom), Region.Op.DIFFERENCE);
